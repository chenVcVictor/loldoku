...
function App() {
  ...
  const [correctedMatrix, setCorrectedMatrix] = useState([[false, false, false], 
                                                          [false, false, false], 
                                                          [false, false, false]])
  ...
  const ansAllCorrect = correctedMatrix.every(row => row.every(element => element == true))

  
                                                        
  
  // const [labelMap, setLabelMap] = useState(createMap(4, 4)) // map of button key to labels
  ...
  useEffect(() => {
    ...
    .then((dataObject) => {
      ...
      // FROM ONLINE ON SYNCING LABEL DICTIONARY WITH LABELMAP
      // const updatedMap = labelMap.map(row => [...row]); // Create a shallow copy of labelMap
      // Object.entries(dataObject.labels).forEach(([key, value]) => {
      //   const [row, col] = key.split('-').map(Number); // Split the key and convert to numbers
      //   if (updatedMap[row] && updatedMap[row][col] !== undefined) {
      //     updatedMap[row][col] = value; // Update the specific cell if it exists
      //   }
      // });

      // setLabelMap(updatedMap);
    })
    }, [])
  ...
  return (
    ...
    >
      <div
      className = "bg-slate-900 rounded-lg flex flex-col items-center"
      >
        <ButtonMatrix
